...
COMPOSE=(docker compose --project-name "$PROJECT_NAME" --env-file "$DEPLOY_DIR/.env" -f "$DEPLOY_DIR/deploy/docker-compose.yml" -f "$DEPLOY_DIR/docker-compose.override.yml" -f "$DEPLOY_DIR/deploy/cluster/docker-compose.yml")
...
"${COMPOSE[@]}" up -d --scale "sub2api=$CANARY_REPLICAS" gateway sub2api
"${COMPOSE[@]}" wait gateway sub2api
...
"${COMPOSE[@]}" up -d --scale "sub2api=$REPLICAS" gateway sub2api
"${COMPOSE[@]}" wait gateway sub2api
curl --fail --silent --show-error --max-time 20 http://127.0.0.1:18080/health >/dev/null
