...
func TestBuildCodexModelsManifestForGroupUsesProviderImageCapabilities(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:  "custom OpenAI-compatible host without modality metadata",
			model: "gpt-5.6-sol",
			accounts: []Account{{
...
func TestBuildCodexModelsManifestForGroupPrefersSyncedOpenAIImageCapabilities(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "explicit text-only snapshot narrows multimodal capability",
			accounts: []Account{
				newAccount(30, []string{"text", "image"}),
				newAccount(31, []string{"text"}),
			},
			modalities: []any{"text"},
		},
		{
			name: "unknown compatible capability narrows multimodal snapshot",
			accounts: []Account{
				newAccount(32, nil),
		...
		{
			name: "all compatible accounts explicitly support images",
			accounts: []Account{
				newAccount(34, []string{"text", "image"}),
				newAccount(35, []string{"text", "image"}),
			},
			modalities: []any{"text", "image"},
		},
	}
	...
	for i, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, account := range tt.accounts {
				require.True(t, account.IsModelSupported("gpt-5.6-sol"), "fixture must participate in capability intersection")
			}
			svc := &GatewayService{accountRepo: codexModelsVisibilityAccountRepo{byGroup: map[int64][]Account{
				groupID: tt.accounts,
			}}}
...
func TestAccountCodexImageInputRequiresTrustedFallbackOrExplicitMetadata(t *testing.T) {
	t.Parallel()
	tests := []struct {
		name        string
		platform    string
		...
		{"official default GPT", PlatformOpenAI, AccountTypeAPIKey, "", "gpt-5.4", nil, true},
		{"official explicit URL GPT", PlatformOpenAI, AccountTypeAPIKey, "https://api.openai.com/v1", "gpt-6-astra", nil, true},
		{"official OAuth GPT", PlatformOpenAI, AccountTypeOAuth, "", "gpt-5.4", nil, true},
		{"compatible GPT no metadata", PlatformOpenAI, AccountTypeAPIKey, "https://relay.example.test/v1", "gpt-5.4", nil, false},
		{"compatible Astra no metadata", PlatformOpenAI, AccountTypeAPIKey, "https://relay.example.test/v1", "gpt-6-astra", nil, false},
		{"compatible GPT empty modalities", PlatformOpenAI, AccountTypeAPIKey, "https://relay.example.test/v1", "gpt-5.4", []string{}, false},
		{"compatible Astra explicit text", PlatformOpenAI, AccountTypeAPIKey, "https://relay.example.test/v1", "gpt-6-astra", []string{"text"}, false},
		{"compatible unknown explicit image", PlatformOpenAI, AccountTypeAPIKey, "https://relay.example.test/v1", "custom-vision", []string{"text", "image"}, true},
		{"official GPT explicit text", PlatformOpenAI, AccountTypeAPIKey, "", "gpt-5.4", []string{"text"}, false},
		{"official Astra stale text", PlatformOpenAI, AccountTypeAPIKey, "", "gpt-6-astra", []string{"text"}, true},
		{"DeepSeek vision", PlatformDeepseek, AccountTypeAPIKey, "https://api.deepseek.com", "deepseek-v4-flash-vision-exp", nil, true},
		{"DeepSeek explicit text", PlatformDeepseek, AccountTypeAPIKey, "https://api.deepseek.com", "deepseek-v4-flash-vision-exp", []string{"text"}, false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			account := &Account{
			...
			if tt.modalities != nil {
				account.SetUpstreamModelMetadataSnapshot(UpstreamModelMetadataSnapshot{
					Models: map[string]UpstreamModelMetadata{
						tt.model: {ID: tt.model, InputModalities: tt.modalities},
					},
				})
			}
			require.Equal(t, tt.want, accountCodexModelSupportsImageInput(account, tt.model))
		})
	}
}
...
func TestCompleteAPIKeyCodexModelsManifestForClientRequiresTrustedGPTImageFallback(t *testing.T) {
	t.Parallel()
	...
	for _, tt := range []struct {
		...
		{"compatible IDs alone", "https://openai-compatible.example.test/v1", false, []any{"text"}},
		{"compatible explicit image metadata", "https://openai-compatible.example.test/v1", true, []any{"text", "image"}},
		{"official known GPT fallback", "https://api.openai.com/v1", false, []any{"text", "image"}},
	} {
		t.Run(tt.name, func(t *testing.T) {
			svc := &OpenAIGatewayService{}
			...
			account := newCodexModelsAPIKeyTestAccount(tt.baseURL)
			if tt.explicitImage {
				account.SetUpstreamModelMetadataSnapshot(UpstreamModelMetadataSnapshot{
					Models: map[string]UpstreamModelMetadata{
						"gpt-5.6-sol": {ID: "gpt-5.6-sol", InputModalities: []string{"text", "image"}},
					},
				})
			}
			...
			require.NoError(t, svc.CompleteAPIKeyCodexModelsManifestForClient(manifest, account))
			models := decodeCodexManifestModels(t, manifest.Body)
			official := tt.baseURL == "https://api.openai.com/v1"
			if official {
				require.Len(t, models, 2)
			} else {
				require.Len(t, models, 3)
			...
			require.Equal(t, tt.want, bySlug["gpt-5.6-sol"]["input_modalities"])
			if official {
				require.NotContains(t, bySlug, "company-coding-model")
				require.Equal(t, []any{"text", "image"}, bySlug["gpt-4o"]["input_modalities"])
			} else {
