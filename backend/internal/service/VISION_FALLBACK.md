...
进一步回归确认，原实现把本地预算超时、空描述和未完成响应统一转换为客户端
502/504 后，再把转换后的状态送进账号健康熔断。三种情况都能在测试中错误触发
整账号冷却。现在健康归因保留原始错误和本地 context 原因；语义失败仍记入
调度结果，真实上游 HTTP 5xx 仍可触发账号健康熔断。

09:54:39 再次通过管理 API 调整并读回运行时策略：优先使用
`gpt-5.6-luna`、`gpt-5.6-sol`、`gpt-6-astra`，允许其他助手作为后备，
单助手 60 秒、整轮 180 秒。合成仪表盘直接描述实测 Luna 20.74 秒、Sol 27.78 秒，
说明此前 15 秒预算会取消正常请求。原策略快照保存在操作端
`/tmp/sub2api-vision-policy-before-20260923.json`。

公网 Responses 双图请求 `e01b73af-f027-4513-8f84-b24703c37174` 在 20.315 秒
以 `response.completed` 结束，正确返回 `ORBIT-7319`、25 行、`COMET-8246`
及紫/橙/绿顺序；主模型由 WorkBuddy 账号 844 执行。
Chat Completions 请求 `c7f26b95-1131-4a3a-a0dd-3daceb6ac04a` 也通过双图验证。
修复前的回归测试对三种本地失败均失败，修复后的 service/handler 视觉、WorkBuddy
和账号健康定向测试全部通过。
