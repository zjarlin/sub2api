...
func TestOpenAIGatewayServiceParseOpenAIImagesRequest_AllowsAgnesImageModel(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body := []byte(`{"model":"agnes-image-2.1-flash","prompt":"draw a cat","size":"1024x768"}`)
...
func TestAccountSupportsOpenAIEndpointCapability(t *testing.T) {
	t.Run("OpenAI APIKey 默认兼容 chat embeddings 和 videos", func(t *testing.T) {
		account := &Account{
			Platform: PlatformOpenAI,
	...
	t.Run("显式列表支持同时声明 chat embeddings 和 videos", func(t *testing.T) {
		account := &Account{
			Platform: PlatformOpenAI,
			Type:     AccountTypeAPIKey,
			Credentials: map[string]any{
				"openai_capabilities": []any{"chat_completions", "embeddings", "videos"},
			},
		}
	...
	t.Run("显式列表只声明 chat 时不支持 embeddings 和 videos", func(t *testing.T) {
		account := &Account{
			Platform: PlatformOpenAI,
			Type:     AccountTypeAPIKey,
			Credentials: map[string]any{
	...
	t.Run("显式 map 支持单独关闭 chat 和 videos 并开启 embeddings", func(t *testing.T) {
		account := &Account{
			Platform: PlatformOpenAI,
			Type:     AccountTypeAPIKey,
			Credentials: map[string]any{
	...
	t.Run("显式 map 可单独开启 videos", func(t *testing.T) {
		account := &Account{
			Platform: PlatformOpenAI,
			Type:     AccountTypeAPIKey,
			Credentials: map[string]any{
				"openai_capabilities": map[string]bool{
					"chat_completions": false,
					"embeddings":       false,
					"videos":           true,
				},
