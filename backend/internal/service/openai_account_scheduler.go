	...
	if accountID <= 0 {
		...
		if err != nil {
			if isOpenAISelectionAbortError(err) {
				return nil, false, newOpenAISelectionError(
...
func (s *defaultOpenAIAccountScheduler) tryAcquireOpenAISelectionOrder(
	...
	attemptStats *openAISelectionAttemptStats,
) (*AccountSelectionResult, bool, error) {
	...
	for i := 0; i < len(selectionOrder); i++ {
		...
		if acquireErr != nil {
			if isOpenAISelectionAbortError(acquireErr) {
				return nil, compactBlocked, newOpenAISelectionError(
					"slot_acquisition",
...
func (s *defaultOpenAIAccountScheduler) selectByLoadBalance(
	...
	result, compactBlocked, acquireErr := s.tryAcquireOpenAISelectionOrder(ctx, req, selectionOrder, explicitModelScope, &attemptStats)
	if acquireErr != nil {
	...
	if s.service.concurrencyService != nil {
		if freshLoadMap, loadErr := s.service.concurrencyService.GetAccountsLoadBatchFresh(ctx, loadReq); loadErr == nil {
			...
			if len(freshPlan.selectionOrder) > 0 {
				freshResult, freshCompactBlocked, freshAcquireErr := s.tryAcquireOpenAISelectionOrder(ctx, req, freshPlan.selectionOrder, explicitModelScope, &attemptStats)
...
func (s *defaultOpenAIAccountScheduler) isAccountRequestCompatible(ctx context.Context, account *Account, req OpenAIAccountScheduleRequest) bool {
	...
	if req.RequestedModel != "" && !isOpenAIAccountModelSupportedForScheduling(account, req.RequestedModel) {
		return false
	}
