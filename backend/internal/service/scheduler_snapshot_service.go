...
func (s *SchedulerSnapshotService) ListSchedulableAccounts(ctx context.Context, groupID *int64, platform string, hasForcePlatform bool) ([]Account, bool, error) {
	useMixed := len(MixedSchedulingSourcePlatforms(platform)) > 0 && !hasForcePlatform
...
func (s *SchedulerSnapshotService) handleBulkAccountEvent(ctx context.Context, payload map[string]any, seen map[batchSeenKey]struct{}) error {
	...
	for _, account := range accounts {
		...
		case PlatformAnthropic, PlatformGemini, PlatformOpenAI, PlatformAntigravity, PlatformGrok,
			...
			PlatformDoubao, PlatformTraework, PlatformWorkbuddy, PlatformZcode, PlatformQoder, PlatformLaya, PlatformJev:
...
func schedulerSnapshotPlatforms() [17]string {
	return [17]string{PlatformAnthropic, PlatformGemini, PlatformOpenAI, PlatformAntigravity, PlatformGrok, PlatformKimi, PlatformZhipu, PlatformDeepseek, PlatformMiniMax, PlatformOpenCodeGo, PlatformDoubao, PlatformTraework, PlatformWorkbuddy, PlatformZcode, PlatformQoder, PlatformLaya, PlatformJev}
}
...
func (s *SchedulerSnapshotService) loadAccountsFromDB(ctx context.Context, bucket SchedulerBucket, useMixed bool) ([]Account, error) {
	...
	if useMixed {
		platforms := append([]string{bucket.Platform}, MixedSchedulingSourcePlatforms(bucket.Platform)...)
		...
		for _, acc := range accounts {
			if acc.Platform == bucket.Platform {
				filtered = append(filtered, acc)
				continue
			}
			if acc.IsMixedSchedulingEnabled() && mixedSchedulingTargetsPlatform(acc.Platform, bucket.Platform) {
				filtered = append(filtered, acc)
			}
		}
		return filtered, nil
