...
func TestAllowOpenAICompatibleMessagesDispatch_CNProvidersExempt(t *testing.T) {
	...
	for _, platform := range []string{service.PlatformKimi, service.PlatformZhipu, service.PlatformDeepseek, service.PlatformMiniMax, service.PlatformOpenCodeGo, service.PlatformDoubao, service.PlatformGrok, service.PlatformZcode} {
...
func TestAllowOpenAICompatibleMessagesDispatch_CompositeResolvedTargets(t *testing.T) {
	...
	for _, model := range []string{"doubao-chat-turbo", "grok-4.3", "kimi-k2-thinking", "glm-5.2", "deepseek-v3.2"} {
...
func TestResolveOpenAIMessagesDispatchMappedModel_CompositeCNTargetsSkipGroupMapping(t *testing.T) {
	...
	for _, model := range []string{"doubao-chat-turbo", "kimi-k2-thinking", "glm-5.2", "deepseek-v3.2", "grok-4.3"} {
		c, _ := gin.CreateTestContext(httptest.NewRecorder())
		c.Request = httptest.NewRequest("POST", "/v1/messages", nil)
