...
func (r *apiKeyRepository) Create(ctx context.Context, key *service.APIKey) error {
	builder := r.client.APIKey.Create().
		...
		SetPersonalAccountScope(key.PersonalAccountScope).
...
func (r *apiKeyRepository) GetByKeyForAuth(ctx context.Context, key string) (*service.APIKey, error) {
	m, err := r.activeQuery().
		...
		Select(
			...
			apikey.FieldUserID,
			apikey.FieldGroupID,
			apikey.FieldPersonalAccountScope,
			apikey.FieldName,
			apikey.FieldStatus,
...
func (r *apiKeyRepository) ListByUserID(ctx context.Context, userID int64, params pagination.PaginationParams, filters service.APIKeyListFilters) ([]service.APIKey, *pagination.PaginationResult, error) {
	...
	if filters.GroupID != nil {
		if *filters.GroupID == 0 {
			q = q.Where(apikey.GroupIDIsNil(), apikey.PersonalAccountScopeEQ(false))
		} else {
			q = q.Where(apikey.GroupIDEQ(*filters.GroupID))
		}
	}
	if filters.PersonalAccountScope != nil {
		q = q.Where(apikey.PersonalAccountScopeEQ(*filters.PersonalAccountScope))
	}
...
func apiKeyEntityToService(m *dbent.APIKey) *service.APIKey {
	...
	out := &service.APIKey{
		...
		GroupID:              m.GroupID,
		PersonalAccountScope: m.PersonalAccountScope,
		Quota:                m.Quota,
...
func groupEntityToService(g *dbent.Group) *service.Group {
	...
	out := &service.Group{
		...
		RPMLimit:                        g.RpmLimit,
		ExposeScheduledAccountInLogs:    g.ExposeScheduledAccountInLogs,
		CreatedAt:                       g.CreatedAt,
		UpdatedAt:                       g.UpdatedAt,
	}
	out.PrepareRuntimeCaches()
	return out
}
