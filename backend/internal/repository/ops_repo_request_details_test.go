	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			rows := sqlmock.NewRows([]string{
				...
				"status_code", "error_id", "phase", "severity", "message", "user_id", "user_email", "username", "user_account", "api_key_id", "account_id", "group_id", "stream",
			}).
				AddRow("success", start, "req-slow", "openai", "gpt-5.5", 12000, 800, nil, nil, nil, nil, nil, 1, "user@example.com", "user", "user", 2, 3, 4, true).
				AddRow("error", start, "req-zero", "openai", "gpt-5.5", 9000, 0, 502, 5, "upstream", "error", "failed", 1, "user@example.com", "user", "user", 2, 3, 4, true).
				AddRow("success", start, "req-missing", "openai", "gpt-5.5", 5000, nil, nil, nil, nil, nil, nil, 1, "user@example.com", "user", "user", 2, 3, 4, false)
