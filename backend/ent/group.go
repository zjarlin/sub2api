...
type Group struct {
	...
	// 边缘计算视觉服务单次价格（USD/次）；nil 表示使用默认价
...
func (*Group) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case group.FieldRateMultiplier, group.FieldPeakRateMultiplier, group.FieldDailyLimitUsd, group.FieldWeeklyLimitUsd, group.FieldMonthlyLimitUsd, group.FieldImageRateMultiplier, group.FieldImagePrice1k, group.FieldImagePrice2k, group.FieldImagePrice4k, group.FieldBatchImageDiscountMultiplier, group.FieldBatchImageHoldMultiplier, group.FieldVideoRateMultiplier, group.FieldVideoPrice480p, group.FieldVideoPrice720p, group.FieldVideoPrice1080p, group.FieldWebSearchPricePerCall, group.FieldVisionPricePerCall, group.FieldSearchPricePer1k, group.FieldAudioRealtimePricePerMin, group.FieldAudioTtsPricePerMillionChars, group.FieldAudioSttPricePerHour, group.FieldProfitMinMargin, group.FieldProfitSafetyBuffer:
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldVisionPricePerCall:
			if value, ok := values[i].(*sql.NullFloat64); !ok {
				return fmt.Errorf("unexpected type %T for field vision_price_per_call", values[i])
			} else if value.Valid {
				_m.VisionPricePerCall = new(float64)
				*_m.VisionPricePerCall = value.Float64
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	if v := _m.VisionPricePerCall; v != nil {
		builder.WriteString("vision_price_per_call=")
		builder.WriteString(fmt.Sprintf("%v", *v))
	}
	builder.WriteString(", ")
