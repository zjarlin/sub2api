...
type APIKey struct {
	...
	// Dispatch this API key only to private accounts owned by the key user
	PersonalAccountScope bool `json:"personal_account_scope,omitempty"`
...
func (*APIKey) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case apikey.FieldIPWhitelist, apikey.FieldIPBlacklist:
			values[i] = new([]byte)
		case apikey.FieldPersonalAccountScope:
	...
	for i := range columns {
		...
		case apikey.FieldPersonalAccountScope:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field personal_account_scope", values[i])
			} else if value.Valid {
				_m.PersonalAccountScope = value.Bool
			}
...
func (_m *APIKey) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("personal_account_scope=")
	builder.WriteString(fmt.Sprintf("%v", _m.PersonalAccountScope))
	builder.WriteString(", ")
