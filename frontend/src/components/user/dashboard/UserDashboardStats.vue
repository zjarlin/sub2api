    ...
    <div v-if="!isSimple" class="card p-4">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-xl font-bold text-emerald-600 dark:text-emerald-400">{{ formatUserCurrency(balance) }}</p>
      ...
      >
        <div class="flex items-center justify-between">
          ...
          <span class="font-mono text-sm text-purple-600 dark:text-purple-400" :title="t('dashboard.actual')">
            {{ USER_CURRENCY_SYMBOL }}{{ formatCost(item.total_actual_cost) }}
          </span>
        </div>
        <div class="mt-2 space-y-1 text-xs">
          <div class="flex items-center justify-between">
            <span class="text-gray-500 dark:text-gray-400">{{ t('dashboard.todayCost') }}</span>
            <span class="font-mono text-gray-900 dark:text-white">{{ USER_CURRENCY_SYMBOL }}{{ formatCost(item.today_actual_cost) }}</span>
          </div>
          <div class="flex items-center justify-between">
              ...
              <template v-else>
                <div class="flex items-center justify-between text-xs">
                  ...
                  <span class="font-mono text-gray-700 dark:text-gray-200">
                    {{ USER_CURRENCY_SYMBOL }}{{ formatQuotaAmount((quotaVal(item.quota, `${w}_usage_usd`) as number) ?? 0) }} / {{ USER_CURRENCY_SYMBOL }}{{ formatQuotaAmount(quotaVal(item.quota, `${w}_limit_usd`) as number) }}
                  </span>
                </div>
                <div class="h-1.5 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-700">
                  <div
...
const quotaAmountFormatter = new Intl.NumberFormat('en-US', {
  minimumFractionDigits: 2,
  maximumFractionDigits: 2,
})
function formatQuotaAmount(n: number): string {
  if (!Number.isFinite(n)) return '0.00'
  return quotaAmountFormatter.format(n)
}
