...
describe('PaymentView recharge rate preview', () => {
  it('uses the selected payment method currency in both locale templates', async () => {
    ...
    expect(en.payment.rechargeRatePreview).toBe('Each 1 {currency} paid credits ¥{amount} to your balance')
    expect(zh.payment.rechargeRatePreview).toBe('每支付 1 {currency}，到账余额 ¥{amount}')
  })
})
