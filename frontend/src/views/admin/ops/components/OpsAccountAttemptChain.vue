      ...
      <li v-for="(attempt, index) in attempts" :key="index">
        ...
        >
          <summary class="flex cursor-pointer list-none flex-wrap items-center gap-2 p-3 text-sm [&::-webkit-details-marker]:hidden">
            <Icon name="chevronRight" size="xs" :stroke-width="2" class="shrink-0 transition-transform group-open:rotate-90" />
            ...
            <span v-if="attempt.isFinalFailure" class="rounded bg-red-100 px-2 py-0.5 text-xs font-medium text-red-800 dark:bg-red-500/20 dark:text-red-200">
          ...
          <div class="border-t border-current/10 px-3 pb-3 pt-2">
            <p v-if="attempt.dropped > 0" class="mb-2 text-xs text-amber-800 dark:text-amber-200">{{ t('admin.ops.errorDetail.attemptChain.dropped', { count: attempt.dropped }) }}</p>
...
const attempts = computed(() => {
  ...
  if (finalFailure && attempts.length > 0) attempts[attempts.length - 1].isFinalFailure = true
  return attempts
})

function accordionClass(attempt: { isFinalFailure: boolean }): string {
  if (attempt.isFinalFailure) return 'border-red-200 bg-red-50 text-red-950 dark:border-red-500/30 dark:bg-red-950/30 dark:text-red-50'
