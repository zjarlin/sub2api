export default {
    accounts: {
      ...
      qoder: {
        title: 'Qoder',
        connectionHint: '直接调用 Qoder 官方 Model Server（api2-v2.qoder.sh）。填写 Qoder 账号的访问令牌，无需配置 Base URL。',
        baseUrlHint: '无需填写，固定使用 Qoder 官方 Model Server。',
        apiKeyHint: '填写 Qoder 账号的访问令牌（Bearer Token）。',
        oauthHint: '设备流授权',
        oauthDesc: '点击下方按钮打开 Qoder 授权页，登录并选择账号后本页会自动完成接入，无需手动复制令牌。',
        openAuthPage: '打开 Qoder 授权页',
        reopenAuthPage: '重新打开授权页',
        waitingAuthorization: '正在等待授权完成…',
        failedToGenerateUrl: '生成 Qoder 授权链接失败',
        pollFailed: '查询 Qoder 授权结果失败',
        authorizationTimeout: '授权超时，请重新发起授权',
        testMode: '测试模式',
        testModeHint: '提交消息模式会复现 Qoder IDE / CLI 生成提交消息的请求：POST /model/v1/chat/completions，stream=true，附带 X-Request-ID / X-Session-ID。',
        testModeCommitMessage: '生成提交消息（真实请求契约）',
        testModeDefault: '普通连接测试',
        diffLabel: 'Git Diff（留空使用示例 diff）',
        diffPlaceholder: '粘贴要生成提交消息的 git diff',
        diffHint: '直接调用 Qoder 官方 Model Server，使用与 Qoder IDE / CLI 生成提交消息相同的请求体与请求头。',
      },
      requestCurl: '实际请求（cURL，令牌已脱敏）：',
      laya: {
        title: 'Laya',
