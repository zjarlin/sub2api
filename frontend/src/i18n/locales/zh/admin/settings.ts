export default {
  ...
  vision: {
    ...
    curl: {
      title: '粘贴并解析 curl',
      description: '支持浏览器 Copy as cURL 的换行、单双引号、JSON、GET 参数与 multipart 表单。认证头只用于识别请求结构，生成时统一替换为变量。',
      placeholder: 'curl -X POST https://upstream.example/v1/chat/completions \\\n  -H \'Content-Type: application/json\' \\\n  -d \'{\'{\'}"model":"your-model","messages":[]{\'}{\'}\'',
      parse: '解析 curl',
      parsed: '已解析，可在下方逐项编辑',
      parseFailed: 'curl 解析失败',
      warnings: '有 {count} 个 curl 选项未被完整导入，已在解析时忽略。',
    },
    request: {
      title: '请求参数与请求头',
    ...
    workbench: {
      ...
      headers: '请求头',
      headersHint: '认证头会在生成时替换为 $SUB2API_KEY。',
      headerName: 'Header',
      headerValue: 'Value',
      noHeaders: '暂无请求头',
      query: 'Query 参数',
      queryHint: '生成时会自动拼接完整 URL。',
      queryName: '参数名',
      queryValue: '参数值',
      noQuery: '暂无 Query 参数',
      body: '请求体',
      bodyHint: 'JSON 会保留缩进；form 使用 key=value 与 & 分隔。',
      formatJson: '格式化 JSON',
    ...
    register: {
      title: '注册到网关可用模型',
      description: '候选模型来自该分组的平台默认模型和可调度账号的模型映射。保存时只更新分组模型白名单。',
      group: '目标分组',
      selectGroup: '请选择分组',
      model: '要注册的模型',
      modelPlaceholder: '例如: laya-multilingual',
      gatewayPath: '网关路径',
      keepImportedPath: '跟随导入 URL',
      useImportedModel: '使用导入的模型名',
      loadCandidates: '加载可用模型',
      availableModels: '可用调度账号模型',
      selectCandidateHint: '点击候选模型会同时更新生成 curl 的 model',
      chooseGroupFirst: '请先选择目标分组',
      noCandidates: '当前分组没有可读取的候选模型',
      replaceHint: '替换现有白名单（谨慎）：默认只合并，保留分组已有模型。',
      register: '注册模型',
      saved: '已保存到分组模型白名单，共 {count} 个模型。',
      loadFailed: '加载可用模型失败',
      saveFailed: '注册模型失败',
      copyCurl: '复制结果 curl',
    },
    output: {
      title: '生成网关 curl',
      description: '输出使用当前请求配置，密钥替换为 $SUB2API_KEY，可直接复制到终端。',
      empty: '解析或编辑请求后，这里会显示生成的网关 curl。',
    },
    endpoints: '可用端点',
    endpointMatrix: '端点矩阵',
    endpointMatrixDescription: '点击任意方块查看请求结构，并直接通过网关发起测试。',
    openEndpoint: '打开端点',
    closeDetail: '关闭详情',
    detailTitle: '请求调试器',
    sendRequest: '发送请求',
    sending: '发送中...',
    responseTitle: '响应',
    responseEmpty: '发送请求后，这里会显示响应内容。',
    responseStatus: '状态',
    responseDuration: '耗时',
    responseSize: '大小',
    responseHeaders: '响应头',
    requestFailed: '请求失败',
    apiKeyLabel: '网关 API Key',
    selectApiKey: '选择可用的 API Key',
    noApiKeys: '请先创建一个启用中的 API Key。',
    useImportedRequest: '使用导入请求',
    exampleRequest: '加载示例',
    detailDescription: '像轻量版 Postman 一样编辑请求，并通过网关直接发送测试。',
    categoryVision: '视觉',
    categoryMedia: '媒体',
    categoryDecision: '决策',
