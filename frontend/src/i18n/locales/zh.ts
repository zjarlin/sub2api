export default {
  ...
  myAccounts: {
    title: '我的账号',
    description: '添加你自己维护、按分组参与公共调度的上游账号',
    create: '添加账号',
    edit: '编辑账号',
    searchPlaceholder: '搜索账号名称',
    empty: '暂无个人账号',
    emptyHint: '添加账号后，它会按所选分组参与账号池调度；这个页面只显示你自己的账号。',
    platform: '平台',
    type: '类型',
    status: '状态',
    name: '名称',
    ...
    privateAccountHint: '这个账号会按所选分组参与账号池调度；其他普通用户不能在“我的账号”页面看到它。',
  ...
  keys: {
    ...
    group: '分组',
    noGroup: '无分组',
    personalAccountScope: '我的账号（旧）',
    personalAccountScopeHint: '旧版专用账号池范围；新建密钥请直接选择真实分组。',
    searchGroup: '搜索分组...',
  ...
  admin: {
    ...
    accounts: {
      ...
      filterTabs: {
        title: '筛选标签',
        description: '内置标签用于全部/未分组/真实分组；自定义标签可组合真实分组、显示分组、关键词、名称前缀和高级正则条件。',
        all: '全部',
        manage: '标签设置',
        defaultTab: '默认激活标签',
        addTab: '添加自定义标签',
        label: '标签名称',
        labelPlaceholder: '例如：高优先级 / 轮询备用',
        realGroup: '真实分组筛选',
        noRealGroup: '不限制真实分组',
        displayGroup: '显示分组筛选',
        displayGroupPlaceholder: '输入账号显示分组名称',
        keywordSearch: '关键词筛选',
        keywordSearchPlaceholder: '例如：prod / claude / 海外',
        namePrefix: '名称前缀',
        namePrefixPlaceholder: '例如：prod-',
        searchRegex: '高级正则筛选',
        searchRegexPlaceholder: '例如：gpt|claude|海外',
        validationLabel: '请填写标签名称',
        validationCondition: '请至少配置一个筛选条件',
        validationRegex: '正则表达式无效',
        unnamedTab: '未命名标签'
      },
      oauthType: 'OAuth',
      // Schedulable toggle
      schedulable: '参与调度',
      allSchedulableStates: '全部调度状态',
      schedulableHint: '开启后账号参与API请求调度',
      schedulableEnabled: '调度已开启',
      schedulableDisabled: '调度已关闭',
      failedToToggleSchedulable: '切换调度状态失败',
      batchTestAccounts: '批量测试账号',
      batchTestingProgress: '测试 {done}/{total}',
      batchTestCurrentPageHint: '测试当前页账号；通过则开启调度，不通过则关闭调度',
      batchTestAccountsEmpty: '当前页没有可测试的账号',
      batchTestAccountsDone: '批量测试完成：通过 {passed} 个，失败 {failed} 个',
      batchTestAccountsPartial: '批量测试完成：通过 {passed} 个，失败 {failed} 个；{scheduleFailed} 个账号调度状态更新失败',
      batchTestAccountsFailed: '批量测试账号失败',
      batchTestNoModel: '未找到可用测试模型',
      batchTestStreamNoBody: '测试接口没有返回内容',
      batchTestStreamIncomplete: '测试流未返回完成事件',
      batchTestTimeout: '测试超时',
      ...
      testConnection: '测试连接',
      copyAccount: '复制账号',
      copyAccountSuccess: '已复制账号为 {name}',
      ...
      quickOpenAI: {
        ...
        defaultGroupRequired: '请先在 OpenAI API Key 分组里勾选一个默认分组'
      },
      // OpenAI specific hints
      openai: {
        baseUrlHint: '留空使用官方 OpenAI API',
        apiKeyHint: '您的 OpenAI API Key',
        deepseekBaseUrlHint: 'DeepSeek 默认使用 https://api.deepseek.com，只提供 OpenAI 兼容 chat/completions 路径；Codex 的 Responses 请求由网关转换。',
        ...
        ollamaBaseUrlHint: 'Ollama 默认监听 11434，并暴露 /v1 OpenAI 兼容接口；本机部署可使用 http://127.0.0.1:11434/v1。',
        ollamaApiKeyHint: 'Ollama 本地服务通常不校验 API Key，可留空；如果前面有反代鉴权，则填写对应 token。',
        ...
        chatgptWeb2apiBaseUrlHint: 'ChatGPT-Web2API 默认监听 8080，并通过已登录的 Chrome 会话暴露 OpenAI 兼容 /v1/chat/completions。',
        chatgptWeb2apiApiKeyHint: '仅在 ChatGPT-Web2API 配置了 api_keys 时填写；未配置鉴权时可留空或使用任意占位 token。',
        seedanceBaseUrlHint: 'Seedance 使用 ModelArk 视频生成任务接口。BytePlus 默认：https://ark.ap-southeast.bytepluses.com/api/v3；火山引擎国内默认：https://ark.cn-beijing.volces.com/api/v3。',
        seedanceApiKeyHint: 'ModelArk API Key，会作为 Bearer token 调用 Seedance 视频生成。',
        ...
        upstreamRate: {
          ...
          reading: '测试中',
          siteMode: '站对站模式',
          siteModeHint: '使用此账号在上游后台登录，自动识别 sub2api/new-api；保存后列表会按 2 小时缓存刷新当前 Key 的倍率与余额。',
          consoleBaseUrl: '上游后台地址',
          siteType: '站点类型',
          siteTypeAuto: '自动识别',
          ...
          email: '登录用户名/邮箱',
          password: '登录密码',
          targetApiKey: '目标 API Key',
          loginPath: '登录路径',
          keysPath: 'Key 列表路径',
          baseUrlRequired: '请输入上游后台地址',
          emailRequired: '请输入上游登录用户名或邮箱',
          passwordRequired: '请输入上游登录密码',
          apiKeyRequired: '请输入要匹配倍率的目标 API Key',
          success: '已读取 {site} 倍率 {rate}，分组 {group}，Key {key}',
          failed: '读取上游倍率失败',
          loginSuccess: '连接成功：{site}，登录态 {state}，用户 {user}，余额 {balance}',
          loginFailed: '上游连接测试失败',
          siteModeRequired: '请先开启站对站模式',
          groupSwitchNewAPIRequired: '当前站点类型不支持切换上游分组',
          targetGroup: '目标上游分组',
          targetGroupPlaceholder: '例如 GPT 超低渠道',
          groupSwitchHint: '点击后会登录上游后台并调用远程接口，只修改该 Key 的上游分组。',
          groupSwitchSelectedHint: '当前选择 {group}，倍率 {rate}。',
          loadGroups: '刷新分组',
          loadingGroups: '加载中',
          loadGroupsFailed: '读取上游分组失败',
          groupOptionsEmpty: '未读取到可用分组，可手工输入分组名称。',
          groupSwitch: '切换上游分组',
          groupSwitching: '切换中',
          groupNameRequired: '请输入目标上游分组名称',
          groupSwitchSuccess: '已切换到上游分组 {group}',
          groupSwitchFailed: '切换上游分组失败',
          keyRate: 'Key',
          accountRate: '账号',
          keyGroup: '分组',
          resolveFailed: '读取失败'
        ...
        endpointCapabilitiesDesc:
          '用于调度筛选。文本端点会跟随上方 Responses API 支持显示为 Responses、Chat Completions 或自动模式；Embeddings 控制 /v1/embeddings；Videos 控制 /v1/videos/generations。',
        ...
        disablePassthroughForModelRestriction: '关闭自动透传',
      },
      kiro: {
      ...
      customModelName: '批量添加模型',
      enterCustomModelName: '每行一个模型，也支持逗号、分号、中文标点等常见分隔符',
      addModel: '导入模型',
      bulkModelInputHint: '支持直接粘贴多行文本，也支持用换行、逗号、分号、中文标点批量导入模型。',
      modelExists: '该模型已存在',
      modelCount: '{count} 个模型',
      bulkImportMappings: '批量导入映射',
      ...
      displayGroupsHint: '仅用于管理后台界面显示和筛选，不参与真实调度分组。',
      poolMode: '池模式',
      ...
      batchTestModelsStarted: '开始批量测试账号 {account} 的 {total} 个模型',
      batchTestModelsPruneHint: '测试完成后只保留成功模型，失败模型会从该账号 model_mapping 中删除。',
      batchTestingModel: '[{current}/{total}] 测试模型：{model}',
      batchTestModelPassed: '通过：{model}',
      batchTestModelFailed: '失败：{model} - {error}',
      batchTestModelsSummary: '批量测试完成：通过 {passed} 个，失败 {failed} 个',
      batchTestModelsPruned: '已清理失败模型，当前保留 {count} 个成功模型',
      batchTestModelsNoSuccess: '没有任何模型测试成功，已保留原 model_mapping，未执行清理',
      retry: '重试',
      copyOutput: '复制输出',
      outputCopied: '输出已复制',
    ...
    ops: {
      ...
      errorLog: {
        ...
        details: '详情',
        tempDisable: '临时禁用',
        tempDisableLoading: '禁用中',
        tempDisableSuccess: '已临时禁用调度账号 {id} {minutes} 分钟',
        tempDisableFailed: '临时禁用调度账号失败',
        phase: '阶段',
        id: 'ID：',
        typeUpstream: '上游',
        typeRequest: '请求',
    ...
    settings: {
      ...
      site: {
        ...
        siteName: '站点名称',
        siteNameHint: '显示在公开主页、邮件和页面标题中',
        siteNamePlaceholder: '++0 的 API',
        siteSubtitle: '站点副标题',
        siteSubtitleHint: '显示在公开主页、登录和注册页面',
        siteSubtitlePlaceholder: '一个接口，接上主流 AI 模型和上游账号池',
        homeFeaturesTitle: '主页能力文案',
        homeFeaturesDescription: '配置默认 Three.js 主页下方的三条能力说明',
        homeFeature1Title: '能力一标题',
        homeFeature2Title: '能力二标题',
        homeFeature3Title: '能力三标题',
        homeFeatureDescription: '描述',
        homeFeature1TitlePlaceholder: '一键接入',
        homeFeature1DescriptionPlaceholder: '获取一个 API 密钥，即可调用所有已接入的 AI 模型，无需分别申请。',
        homeFeature2TitlePlaceholder: '稳定可靠',
        homeFeature2DescriptionPlaceholder: '智能调度多个上游账号，自动切换和负载均衡，告别频繁报错。',
        homeFeature3TitlePlaceholder: '用多少付多少',
        homeFeature3DescriptionPlaceholder: '按实际使用量计费，支持设置配额上限，团队用量一目了然。',
        apiBaseUrl: 'API 端点地址',
